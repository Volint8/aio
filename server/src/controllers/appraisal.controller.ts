...
export const exportAppraisalCsv = async (req: Request, res: Response) => {
  try {
    ...
    const header = 'Employee Name,Email,Organization,Cycle,Tasks Completed (%),Deadlines Met (%),OKR Impact Score (%),OKR Contribution,Overall Rating,Status,Created At,OKR Title,KR Title,Metric Unit,Target,Actual,KR Achieved (%)\n';
    ...
    if (okrImpactSummary?.okrs?.length) {
      for (const okr of okrImpactSummary.okrs) {
        ...
        for (const kr of keyResults) {
          rows.push(
            `${baseRowPrefix},"${okr.okrTitle || ''}","${kr.krTitle || ''}","${kr.metricUnit || ''}",${kr.targetValue ?? ''},${kr.actualValue ?? 0},${kr.achievedPct ?? ''}`
          );
        }
      }
    } else {
